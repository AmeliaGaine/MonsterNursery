# CADENCE.md — Stage 3.3 (Cadence Designer lock)

*The per-chapter rhythm for all 49 episodes, scheduled against the IMMUTABLE
`ARCHITECTURE.md`. Honors CANON.md verbatim and GUARDRAILS G4 (cookie law), G5
(anti-plateau), G6 (no cliffhanger cruelty). Arc skeleton 12/12/13/12 = 49.*

> **How to read the columns**
> - **Cookie type** (G4): one of `number-up` · `mystery` (deepened/clue) · `relationship` ·
>   `joke` (landed) · `competence`. No two adjacent chapters repeat the same type.
> - **Out** (G6): `warm` (cozy floor — a synergy-pair beat from SM), `soft-hook` (a System
>   editorial note that implies the next problem), or `hard-hook` (a live clock advances —
>   never a cruel cliffhanger). Alternation: never two hard-hooks in a row; never three
>   warm-outs in a row.
> - **Live clock active** (G5): the live clock OR deepening-mystery confirmed running this
>   chapter. The System voice-drift tachometer (`L-EDDA`/`L-HEIR`) is on by construction in
>   *every* chapter, so this column is `YES` for all 49 — the named clock is the foreground one.
> - **Micro-structure** (3.3): `K&` = Kishōtenketsu (cozy/warm, satisfies without manufactured
>   conflict) · `S&S` = Scene&Sequel (tension). Target ~60% K& / ~40% S&S; every arc climax = S&S.
> - **Upgrade beat?**: a larger sanctuary/progression upgrade (room unlock, rank-up, skill,
>   loop-close that grows the map). Aligned to rooms-bible unlock episodes where sensible;
>   cadence target ~1 per 3 chapters.

---

## ARC 1 — "Becoming a Carer" (Ch 1–12)

*Foreground: ③ Texture + ② Access. Mystery chamber: OUTWARD. Arc-out: WARM (family forms).*

| Ch | Cookie type | Out (hook/warm) | Live clock active | Micro-structure | Upgrade beat? |
|---|---|---|---|---|---|
| 1 | mystery | soft-hook | YES — `L-SERIES`/`L0`/`L-EDDA` open; ledger-spine name; System greets oddly | K& | **YES — Moonwell unlocks (Ep1, canon); Cress arrives; first Hearthglow** |
| 2 | relationship | warm | YES — `L-TRUST` first feeding; Edda voice-drift on | K& | Gentle Handling I (skill) |
| 3 | competence | hard-hook | YES — `L-HEIR` opens (System "wakes" for *this* clerk); `L-TRUST` **closes** | S&S | — (innermost loop pays; door cracks: why her?) |
| 4 | relationship | warm | YES — Cinder intake; heat-is-harm flinch; mystery drift | K& | **YES — Basking Nook unlocks (Ep4, canon); Cinder arrives** |
| 5 | competence | soft-hook | YES — Habitat Stability pressure; "Current Towel: legally insufficient" | S&S | **YES — Welcome Desk unlocks (Ep5); Habitat Mending I; patron intake** |
| 6 | number-up | warm | YES — Compliance ticks; Edda specificity noticed | K& | **YES — rank-up: Custodian I (Moonwell stabilized + first 3 skills)** |
| 7 | relationship | soft-hook | YES — Diagnosis I = *see* every untreated hurt (new ache clock) | K& | Diagnosis I (skill) |
| 8 | mystery | hard-hook | YES — `L-STAB` opens (Bertram mechanic; a wall sags when he sulks) | S&S | **YES — Gargoyle/Habitat-Stability mechanic formalized (Ep8, canon)** |
| 9 | joke | warm | YES — `L-KRAKEN` opens; Toad↔Kraken frenemy dial armed | K& | **YES — Brew Pool unlocks (Ep9, canon); Tea Kraken steeping; Resonance Cooking I** |
| 10 | number-up | soft-hook | YES — `L-CINDER` Tea-Room interview seed; Renown channel; voice-drift | S&S | **YES — Tea Room unlocks (Ep10); matching mechanic; Custodian-I intake load** |
| 11 | relationship | warm | YES — Kraken↔Cress signature corner; System softens to the tea (voice *alive*) | K& | **YES — Resonance Hearth (Ep11); Gentle Handling II; the Arc-1 mystery button** |
| 12 | competence | **warm** (arc climax) | YES — `L0` marker: first inspection (Quill) survived; Quill lets slip "someone wanted this to fail" | **S&S** | **YES — rank-up: Custodian II; Compliance crosses viable** |

**Arc 1 out-rhythm:** soft → warm → hard → warm → soft → warm → soft → hard → warm → soft → warm → **warm (climax)**.
No two hard-hooks adjacent (3, 8 are isolated); longest warm run = 2 (11→12); zero three-warm sags.
**K&/S&S:** 7 K& / 5 S&S = **58% K&**. Arc climax (Ch12) = S&S ✓.
**Anti-plateau:** all 12 `YES`. `L-TRUST` closes Ch3 to vent peak density; mystery drift never off. **No flags.**

---

## ARC 2 — "The Pipeline & the Petition" (Ch 13–24)

*Foreground: ④ Social/Renown + ② Access. Mystery chamber: INWARD. Arc-out: SOFT-HOOK (a name surfaces).*
...
## ARC 3 — "The Archive & the East Wing" (Ch 25–37)

*Foreground: ⑥ Lore/Comprehension + ⑤ Mastery. Mystery chamber: PERSONAL→excavation.
Arc-out: WARM-WITH-ACHE (Rhea; first send-offs). Carries the extra 13th episode.*
...
## ARC 4 — "The Seizure & the Recognition" (Ch 38–49)

*Foreground: ① Magnitude-as-stakes + ④ Social. Mystery chamber: KEYSTONE.
Arc-out: WARM-CLIMAX-THEN-ONE-DOOR. No new rooms (CANON) — everything earned is spent defending home.*
...
### Per-arc hook/warm rhythm (out-types)

| Arc | warm | soft-hook | hard-hook | Arc-out (climax) | Two-hard-adjacent? | Three-warm-run? |
|---|---|---|---|---|---|---|
| Arc 1 (1–12) | 6 | 3 | 2 + 1 climax-warm | **warm** (family forms) | none | none |
| Arc 2 (13–24) | 5 | 5 + 1 climax-soft | 2 | **soft-hook** (a name surfaces) | none | none |
| Arc 3 (25–37) | 6 + 1 climax-warm-ache | 5 | 2 | **warm-with-ache** (Rhea) | none | none |
| Arc 4 (38–49) | 5 + 1 climax-door | 3 | 3 | **warm-climax-then-one-door** | none | none |

**Arc-out alternation (G6):** warm → soft-hook → warm-with-ache → warm-climax-then-door.
No two cruel hooks adjacent; no three warm-outs flat; the Book-1 ender is an *appetite*
(Pike's custodian-transfer filing = the single ajar `L-SERIES` door), not a fall. **PASS.**

### Cookie-law check (G4)

Every chapter pays ≥1 cookie. No two adjacent chapters share a cookie type — verified across
all 48 boundaries (including the three arc seams 12→13, 24→25, 37→38: competence→relationship,
mystery→mystery? **no** — Ch24 = mystery, Ch25 = mystery would clash, so Ch25 is scheduled as
`mystery` only because Ch24's *climax* cookie is logged as `mystery`; **corrected below**).

> **Seam audit (the three arc boundaries):**
> - 12 `competence` → 13 `relationship` ✓
> - 24 `mystery` → 25 `mystery` ✗ → **Ch25 reassigned to `competence`** (sporeglow-management
>   competence-display; the lichen-containment beat is a clean competence cookie). Re-verified:
>   24 `mystery` → 25 `competence` → 26 `relationship` ✓.
> - 37 `mystery` → 38 `mystery` ✗ → **Ch38 reassigned to `competence`** (the defense mobilizes —
>   a competence-display as Liora reads the seizure order and acts). Re-verified:
>   37 `mystery` → 38 `competence` → 39 `competence` ✗ → **Ch39 reassigned to `relationship`**
...
> these into the arc tables; the schedule itself is valid as corrected.)*

### Kishōtenketsu / Scene&Sequel ratio (3.3)
...
Upgrade beats land at Ch **1, 4, 5, 6, 8, 9, 10, 11, 12** (Arc 1) · **13, 16, 18, 20, 22, 24**
(Arc 2) · **26, 28, 30, 31** (Arc 3) · **45, 48, 49** (Arc 4).

- **Room unlocks** aligned to rooms-bible exactly: Moonwell 1 · Basking Nook 4 · Welcome Desk 5 ·
...
- **Cadence ~1/3:** Arc 1 is upgrade-dense (the establish-the-engine arc — front-loaded by canon
  ...
  Regional-recognition number-up, which is the correct shape (stakes, not acquisition).

### Anti-plateau audit (G5)

**All 49 chapters: live clock OR deepening mystery `active` = YES.** The System voice-drift
